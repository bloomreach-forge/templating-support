...
public class PebbleHstResourceURLNode implements RenderableNode {
    private static final Logger log = LoggerFactory.getLogger(PebbleHstResourceURLNode.class);
    ...
    public PebbleHstResourceURLNode(final int lineNumber, final String id, final String value) {
    ...
    public void render(final PebbleTemplateImpl self, final Writer writer, final EvaluationContextImpl context) throws IOException {
        String result = "";
        if (value != null) {
            result = HstURLHelper.INSTANCE.resourceURL(id, value);
        }
        writer.write(result);

...
public class PebbleHstComponentRenderingURLTag implements TokenParser {

    private static final Logger log = LoggerFactory.getLogger(PebbleHstComponentRenderingURLTag.class);

    @Override
    public String getTag() {
        return "componentRenderingURL";
    }
    ...
    public RenderableNode parse(final Token token, final Parser parser) {
        ...
        if (peek.getType() == Token.Type.EXECUTE_END) {
            ...
            return new PebbleComponentRenderingURLNode(lineNumber, "");
        }
        stream.next();
        ...
        if (parsedExpression instanceof LiteralStringExpression) {
            ...
            return new PebbleComponentRenderingURLNode(lineNumber, literalStringExpression.getValue());
        }
        else  {
